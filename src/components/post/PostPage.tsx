import { Box, Button, Grid, Stack, Typography, ListItem , ListItemIcon } from "@mui/material";
...
const PostPage: FC<postPageProps> = ({ article }): ReactElement => {
  ...
  useEffect(()=>{
    const selectBox = textRef.current;
    selectBox.innerHTML = article.sanitizedHtml;
  },[])
  const data = articles[0]
const dayNames =["রবিবার","সোমবার","মঙ্গলবার","বুধবার","বৃহস্পতিবার","শুক্রবার","শনিবার"];
const monthNames = ["জানুয়ারি","ফেব্রুয়ারী","মার্চ","এপ্রিল","মে","জুন","জুলাই","আগস্ট","সেপ্টেম্বর","অক্টোবর","নভেম্বর","ডিসেম্বর"];
...
const bangla_date = (created) => {
  const getAppendZero = (value) => {
    if(value < 10){
  ...
  const banglaFinal = day + " " + month + " " + year + ", " + hour + " : " + minute;
      ...
      <Box>
        ...
        >
          ...
          >
            <img
              src={article.image}
              style={{ maxHeight: "100%", minWidth:"50%", maxWidth: "90%", opacity: "0.7" }}
            />
          </Box>
          <Box sx={{
            minHeight:"300px",
          ...
          <CustomLink href={`/categories/${categoryHref}`}>
            <H5 fontSize="1.3rem" sx={{  p:1, width:"fit-content", position: "relative", opacity: "1", bgcolor:"white", color:"black" }}>
              {article.cat[0]}
            </H5>
            </CustomLink>
            <H1 fontSize="2.5rem" sx={{ position: "relative", p:1.5, bgcolor:"white", color:"black", fontFamily:"solaimanlipi" }}>{article.title}</H1>
          </Box>
          </Box>
        </Box>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <Typography variant="body1" sx={{p:1}}>{bangla_date(article.created)}</Typography>
                </Stack>
              </Stack>
            ...
            <Box sx={{ bgcolor: "text.light", pl:0.5, mx:{xs:0, sm:2}}}>
              <Stack direction="row" spacing={1.5} sx={{p:1}}>
                  ...
                  <Typography>
                    ...
                    Enim deleniti tenetur atque, velit unde animi nemo magnam
                    quo, expedita quibusdam hic alias quam dolores, accusantium
